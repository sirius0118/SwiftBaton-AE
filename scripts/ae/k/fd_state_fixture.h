...
{
    for (size_t i = 0; i < size; i++) buf[i] = (char)(19 + fd_owner * 23 + group * 37 + kind * 11 + i * 13);
}
...
static unsigned fd_verify_queue(int input, int output, unsigned group, unsigned kind, bool socket_fd, bool retain)
{
    ...
    if (socket_fd) {
        ...
        if (getsockopt(input,SOL_SOCKET,SO_TYPE,&type,&len)) fail("FD queue type");
        datagram=type==SOCK_DGRAM;
    }
    ...
    while(received<sizeof(buf)) {
        ...
        struct pollfd ready={.fd=input,.events=POLLIN};
        int available=poll(&ready,1,1000-elapsed);
        if(available<0&&errno==EINTR)continue;
        if(available<=0)break;
        ssize_t n=socket_fd?recv(input,buf+received,sizeof(buf)-received,MSG_DONTWAIT|(datagram?MSG_TRUNC:0)):
                            read(input,buf+received,sizeof(buf)-received);
        if(n<0&&(errno==EAGAIN||errno==EINTR))continue;
        if(n<=0)break;
        if((size_t)n>sizeof(buf)-received) {
            fprintf(stderr,"FD_QUEUE_OVERSIZED owner=%u group=%u kind=%u bytes=%zd\n",fd_owner,group,kind,n);
            return 1;
        }
        received+=n;
        if(datagram)break; /* Preserve datagram boundaries, even on a short message. */
    }
    unsigned bad = received != sizeof(buf) || (received == sizeof(buf) && memcmp(buf, expected, sizeof(buf)));
    if(bad) fprintf(stderr,"FD_QUEUE_BAD group=%u kind=%u bytes=%zu errno=%d\n",group,kind,received,errno);
    ...
    if (received > 0 && retain) {
        ssize_t written = socket_fd ? send(output, buf, received, MSG_NOSIGNAL) : write(output, buf, received);
        bad += written != (ssize_t)received;
        if(written!=(ssize_t)received) fprintf(stderr,"FD_QUEUE_RETURN_BAD group=%u kind=%u bytes=%zd errno=%d\n",group,kind,written,errno);
    }
    return bad;
}
...
static void fd_state_init(void)
{
    ...
    if (!requested) return;
    const char *owner = getenv("SB_AE_FD_OWNER");
    fd_owner = owner ? strtoul(owner, NULL, 10) : 0;
    if (fd_owner > 8) fail("FD owner");
    ...
    for (unsigned i = 0; i < fd_group_count; i++) {
        ...
        uint64_t three = 3;
        g->counter = (UINT64_C(1) << 40) + ((uint64_t)fd_owner << 20) + i + 1;
        ...
        struct epoll_event event = {.events = EPOLLIN, .data.u64 = UINT64_C(0xa100000000) + ((uint64_t)fd_owner << 40) + 2*i};
        ...
        if (getenv("SB_AE_FD_ESCAPED_EPOLL")) {
            /* epoll remembers the original fd number even after it is closed.
             * Keep its OFD alive under another number, then let pipe2 reuse the
             * old number. Dump must resolve the actual watch, not the reused fd. */
            int alias=fcntl(g->normal,F_DUPFD_CLOEXEC,0);
            if(alias<0)fail("FD watched alias");
            close(g->normal);g->normal=alias;
        }
        ...
        g->offset = 4096 + 64*(i + fd_owner*(fd_group_count+1));
        ...
        g->timer_kind = getenv("SB_AE_TIMERFD_VARIANTS") ? i % 4 : 0;
        g->deadline = time(NULL) + 3600;
        struct itimerspec spec = {.it_value.tv_sec = g->deadline};
        if (g->timer_kind == 1) spec.it_value.tv_sec = 0;
        if (g->timer_kind >= 2) {
            g->deadline = time(NULL) - 5;
            spec.it_value.tv_sec = g->deadline;
            if (g->timer_kind == 3) spec.it_interval.tv_sec = 3600;
        }
    ...
    /* The first verification must not race the churn thread's initial setup. */
    for (;;) {
        pthread_mutex_lock(&fd_churn_lock);
        bool ready = fd_churn_epoch != 0;
        pthread_mutex_unlock(&fd_churn_lock);
        if (ready) break;
        usleep(1000);
    }
...
static uint64_t fd_state_verify(void)
{
    ...
    for (unsigned i = 0; i < fd_group_count; i++) {
        struct fd_group *g = &fd_groups[i];
        uint64_t checkpoint = bad;
#define FD_NOTE(stage) do { \
    if (bad != checkpoint) fprintf(stderr,"FD_STATE_BAD owner=%u group=%u stage=%s added=%llu\n", \
        fd_owner,i,stage,(unsigned long long)(bad-checkpoint)); \
    checkpoint = bad; \
} while (0)
        ...
        bad += n != 2;
        for (int j = 0; j < n; j++) {
            uint64_t tag = ready[j].data.u64 - (UINT64_C(0xa100000000) + ((uint64_t)fd_owner << 40) + 2*i);
        ...
        FD_NOTE("file_alias_offsets");
        struct itimerspec spec;
        if (timerfd_gettime(g->timer, &spec)) bad++;
        else if (!g->timer_kind) {
            long long deadline_delta=(long long)time(NULL)+spec.it_value.tv_sec-g->deadline;
            unsigned timer_bad=spec.it_interval.tv_sec || spec.it_interval.tv_nsec || llabs(deadline_delta)>2;
            if(timer_bad) fprintf(stderr,"FD_TIMER_BAD owner=%u group=%u deadline_delta=%lld remaining=%lld.%09ld interval=%lld.%09ld\n",
                fd_owner,i,deadline_delta,(long long)spec.it_value.tv_sec,spec.it_value.tv_nsec,
                (long long)spec.it_interval.tv_sec,spec.it_interval.tv_nsec);
            bad+=timer_bad;
        } else {
            uint64_t ticks=0;
            ssize_t n=read(g->timer,&ticks,sizeof(ticks));
            if(g->timer_kind==1) {
                bad+=n!=-1||errno!=EAGAIN||spec.it_value.tv_sec||spec.it_value.tv_nsec||
                    spec.it_interval.tv_sec||spec.it_interval.tv_nsec;
            } else {
                /* Tests finish within 100 seconds. This period keeps the
                 * read/reinsert verification away from the next expiration;
                 * short-period overrun tests live in the native regression. */
                bad+=n!=sizeof(ticks)||ticks!=1;
                if(n==sizeof(ticks)&&ticks) bad+=ioctl(g->timer,_IOW('T',0,uint64_t),&ticks)!=0;
                if(g->timer_kind==2) bad+=spec.it_value.tv_sec||spec.it_value.tv_nsec||
                    spec.it_interval.tv_sec||spec.it_interval.tv_nsec;
                else {
                    long long delta=(long long)time(NULL)+spec.it_value.tv_sec-(g->deadline+3600);
                    bad+=spec.it_interval.tv_sec!=3600||spec.it_interval.tv_nsec||llabs(delta)>2;
                }
            }
        }
        FD_NOTE("timer");
...
#undef FD_NOTE
    ...
    if (bad != before_churn) fprintf(stderr,"FD_STATE_BAD owner=%u stage=churn added=%llu\n",fd_owner,(unsigned long long)(bad-before_churn));
    fd_state_errors += bad;
    if (!fd_state_errors && fd_group_count >= 4 && getenv("SB_AE_TIMERFD_VARIANTS"))
        printf("SBK_TIMERFD_VARIANTS_PASS owner=%u future_disarmed_expired_periodic=1\n", fd_owner);
