...
parser.add_argument('--numa-node', type=int, choices=[0, 1], help='Bind both containers CPU/memory, destination pageclient and anonymous PS staging to this NUMA node')
...
if opts.numa_node is not None and not (opts.parent_stage and opts.runtime_snapshot):
    parser.error('--numa-node requires --parent-stage and --runtime-snapshot for placement verification')
...
try:
    ...
    if opts.fast_cutover:
        ...
        helper = str(Path(__file__).with_name('fast_cutover.py'))
