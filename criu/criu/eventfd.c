...
{
	static unsigned warned_unknown_mode;
	EventfdFileEntry efd = EVENTFD_FILE_ENTRY__INIT;
	FileEntry fe = FILE_ENTRY__INIT;

	if (parse_fdinfo(lfd, FD_TYPES__EVENTFD, &efd))
		return -1;
	/* Older kernels do not expose ctx->flags. Do not probe with read/write:
	 * that mutates the counter and readiness of the live kernel object. */
	if (!efd.has_semaphore && !__atomic_exchange_n(&warned_unknown_mode, 1, __ATOMIC_RELAXED))
		pr_warn("SB_FD eventfd mode absent from kernel fdinfo; legacy non-semaphore restore assumed\n");
...
static int eventfd_open(struct file_desc *d, int *new_fd)
{
	...
	tmp = sb_eventfd_restore(info->efe->counter, info->efe->has_semaphore && info->efe->semaphore);
	if (tmp < 0) {
