...
/* Parsers run concurrently in the per-process dump workers. Keep a spare
 * terminator byte for read() callers which fill the complete buffer. */
static __thread char buf[BUF_SIZE + 1];
...
int parse_smaps(pid_t pid, struct vm_area_list *vma_area_list, dump_filemap_t dump_filemap)
{
	...
	f.fd = -1;
#ifdef DOCKER
	if (opts.sb_vma_cache && dump_filemap)
		f.fd = sb_vma_cache_open(pid);
#endif
	if (f.fd < 0)
	...
	while (1) {
		...
		bool eof;
		char *str;
		// LONGTIME: 这里，从f中读取数据到缓冲区消耗了大量时间，5GB的物理内存页面（与虚拟内存大小无关，主要影响是物理内存）导致这个操作大概需要120ms时间
		// 但是，实际上获取到的数据并不长，str应该很短。这里的原因是因为Linux，就算是使用cat /proc/<pid>/smaps都会出现上面的问题，linux统计VMA需要较长耗时
		str = breadline(&f);
		if (IS_ERR(str))
			goto err;
		eof = (str == NULL);
...
static int parse_fdinfo_pid_s(int pid, int fd, int type, void *arg)
{
	...
	while (1) {
		...
		if (fdinfo_field(str, "eventfd-semaphore")) {
			EventfdFileEntry *efd = arg;
			unsigned semaphore;
			if (type != FD_TYPES__EVENTFD ||
			    sscanf(str, "eventfd-semaphore: %u", &semaphore) != 1 || semaphore > 1)
				goto parse_err;
			efd->has_semaphore = true;
			efd->semaphore = semaphore;
			continue;
		}
...
int parse_threads(int pid, struct pid **_t, int *_n)
{
	...
	if (*_t)
		t = *_t;
	// 从 /proc/[pid]/task 文件夹中会显示多个子文件夹，每个都是一个线程
