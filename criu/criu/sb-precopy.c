    ...
    while (!__atomic_load_n(&context->error, __ATOMIC_RELAXED) &&
        ...
        while (begin < job->end) {
            ...
            for (size_t i = begin; i < end; i++) {
                ...
                if (source_allowed && !(source_allowed[i / 8] & (1U << (i % 8)))) continue;
                if (!pages[i].copied || !pages[i].pfn || (entry & SB_PFN_MASK) != pages[i].pfn ||
                    !(entry & (1ULL << 63)) || (entry & ((1ULL << 55) | (1ULL << 62)))) continue;
                ...
                if (!matches || !context->eligible(p->pid, pages[i].address)) continue;
