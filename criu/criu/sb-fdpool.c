...
struct hint { uint64_t magic; uint32_t version, counts[SB_FDPOOL_KINDS]; };
struct context {
...
{
    struct hint h={.magic=HINT_MAGIC,.version=1};
    unsigned total=0;
    ...
    pr_info("SB_FDPOOL hint total=%u event=%u semaphore=%u epoll=%u tcp4=%u udp4=%u tcp6=%u udp6=%u\n",
        total,h.counts[0],h.counts[1],h.counts[2],h.counts[3],h.counts[4],h.counts[5],h.counts[6]);
    return 0;
}
...
{
    struct hint h; struct stat st; unsigned total=0;
    int fd=openat(image_dir,HINT_FILE,O_RDONLY|O_CLOEXEC);
    if(fd<0) return errno==ENOENT?0:-1;
    int ok=!fstat(fd,&st) && st.st_size==sizeof(h) && read(fd,&h,sizeof(h))==sizeof(h);
    close(fd);
    if(!ok || h.magic!=HINT_MAGIC || h.version!=1) return -1;
...
void sb_fdpool_populate(int permitted)
{
    ...
    for(unsigned k=0;k<SB_FDPOOL_KINDS;k++) {
        ...
        while(remaining) {
            int fds[SB_FDPOOL_BATCH]; unsigned n=0, wanted=remaining<SB_FDPOOL_BATCH?remaining:SB_FDPOOL_BATCH;
        ...
        pr_info("SB_FDPOOL prepared kind=%u requested=%u created=%u\n",k,pool->hint.counts[k],pool->prepared[k]);
    }
