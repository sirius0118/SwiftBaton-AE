...
{
	int ret;
    if (sb_proc_thread_active()) return sb_proc_thread_set_self(fd);
...
{
    if (sb_proc_thread_active()) { sb_proc_thread_clear(0); return 0; }
	set_proc_self_fd(-1);
...
{
    if (sb_proc_thread_active()) { sb_proc_thread_clear(1); return; }
	close_pid_proc();
...
{
	int _fd;
    if (sb_proc_thread_active()) return sb_proc_thread_set_root(fd);
...
inline int open_pid_proc(pid_t pid)
{
    ...
    if (sb_proc_thread_active()) return sb_proc_thread_open(pid);
...
int cr_system_userns(int in, int out, int err, char *cmd, char *const argv[], unsigned flags, int userns_pid)
{
	...
	int ret = -1, status;
	pid_t pid;
	bool trace_network = !strcmp(cmd, "iptables-restore") || !strcmp(cmd, "ip6tables-restore");
	...
	if (trace_network)
		sb_trace("network.helper_fork_begin");
	pid = fork();
	if (trace_network && pid != 0)
		sb_trace("network.helper_fork_done");
	if (pid == -1) {
...
ssize_t read_all(int fd, void *buf, size_t size)
{
	ssize_t n = 0;
	while (size > 0) {
		// LONGTIME:这一行，由于读取这个很大的VMA的信息，会导致时间很长。
