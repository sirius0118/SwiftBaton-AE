...
static inline void mutex_lock(mutex_t *m)
{
	...
	/* Acquire protects the data guarded by this mutex, not only its word. */
	while ((c = __atomic_add_fetch(&m->raw.counter, 1, __ATOMIC_ACQUIRE)) != 1) {
...
{
	uint32_t c = 0;
	/* A plain atomic_set allowed protected stores to move past unlock. */
	__atomic_store_n(&m->raw.counter, (int)c, __ATOMIC_RELEASE);
