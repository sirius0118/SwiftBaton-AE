...
/* CRIU supplies its strong barrier pointer; standalone PS helpers do not. */
extern struct futex_barriers *barriers __attribute__((weak));
static mutex_t standalone_status_mutex;
#endif
...
#ifdef PARALLEL_DUMP
		{
			mutex_t *cleanup_lock = (&barriers && barriers) ?
				&barriers->mutex1 : &standalone_status_mutex;
...
/* Source registration during PS can use Compel before CRIU's dump
	 * barriers exist. Allocate a unique socket nonce without that global. */
	uint64_t socket_id = __atomic_fetch_add(&compel_run_id, 1, __ATOMIC_RELAXED);
	saddr->sun_family = AF_UNIX;
	snprintf(saddr->sun_path, UNIX_PATH_MAX, "X/crtools-pr-%d-%" PRIx64, key, socket_id);
...
/*这里我们需要创建多个socket fd，而不是复用已有的*/
#ifdef PARALLEL_DUMP
	// 这里需要沿用之前的代码，因为这里需要使用的socket必须与目标进程在同一个网络命名空间中，因此在外面创建
	ssock = ctl->ictx.sock;
	...
	// 当没有新的socket的时候，也就是无法复用已有sockfd，这里会返回0，然后bind一个socket，然后listen
	if (sk_len == sizeof(addr.sun_family)) {
		if (bind(ssock, (struct sockaddr *)&args->h_addr, args->h_addr_len) < 0) {
			pr_perror("Can't bind socket");
			goto err;
		}
	// pr_err("绑定地址len:%ld, len:%d, path:%s...\n", sizeof(addr.sun_family), args->h_addr.sun_family,args->h_addr.sun_path);
	// pr_err("绑定地址len:%ld, len:%d, path:%s...\n", sizeof(addr.sun_family), args->h_addr.sun_family,args->h_addr.sun_path + 1);
		if (listen(ssock, 1)) {
...
int compel_cure_remote(struct parasite_ctl *ctl)
{
	...
	if (!ctl->remote_map)
		return 0;
	// 调用mumap操作将parasite code代码从内存空间中移除
