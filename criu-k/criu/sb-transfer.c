...
int sb_parallel_negotiate(int socket)
{
    struct { uint64_t magic; uint32_t version, enabled; } local = {
        ...
        (opts.sb_no_hot_first << 2) | (opts.sb_no_pretransfer << 3) | (opts.sb_kernel_transfer << 4) }, remote;
    ...
    if (memcmp(&local, &remote, sizeof(local)) ||
        ...
        (!opts.sb_parallel_transfer && !opts.sb_kernel_transfer && (opts.sb_no_prefetch || opts.sb_no_hot_first || opts.sb_no_pretransfer)) ||
