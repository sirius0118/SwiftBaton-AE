...
/* Lock order: file_id_lock -> fd_tree.lock, never the reverse. This serializes
 * the inode cache's check-and-insert and the ID association, not file dumping. */
static pthread_mutex_t file_id_lock = PTHREAD_MUTEX_INITIALIZER;
...
static int fd_id_generate_special_locked(struct fd_parms *p, u32 *id)
{
	if (p) {
		struct fd_id *fi;
...
static int fd_id_generate_locked(pid_t pid, FdinfoEntry *fe, struct fd_parms *p)
{
	u32 id;
	struct kid_elem e;
	int new_id = 0;
...
{
    int ret;
    pthread_mutex_lock(&file_id_lock);
    ret = fd_id_generate_special_locked(p, id);
    pthread_mutex_unlock(&file_id_lock);
    return ret;
}
int fd_id_generate(pid_t pid, FdinfoEntry *fe, struct fd_parms *p)
{
    int ret;
    pthread_mutex_lock(&file_id_lock);
    ret = fd_id_generate_locked(pid, fe, p);
    pthread_mutex_unlock(&file_id_lock);
    return ret;
}
