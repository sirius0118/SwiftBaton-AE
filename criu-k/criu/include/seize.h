...
/* K workers already released their own ptrace tasks; finish only freezer state. */
extern int pstree_finish_freezer(int st);
extern const char *get_real_freezer_state(void);
extern bool alarm_timeouted(void);

#ifdef PARALLEL_DUMP
extern void thread_pstree_switch_state(struct pstree_item *item, int st);
#endif
