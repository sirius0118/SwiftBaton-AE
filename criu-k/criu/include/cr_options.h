...
struct cr_options {
	...
	unsigned sb_kernel_timeout_ms;
	unsigned sb_kernel_export_workers, sb_kernel_export_chunk_mb;
	unsigned sb_kernel_ps_chunk_mb; /* Independently published PS snapshot span; 0 retains legacy 4 GiB. */
	bool sb_u_precopy;
	...
	unsigned sb_rdma_mtu; /* Zero negotiates the active endpoint MTUs. */
	bool sb_fault_trace; /* Buffer per-request timestamps; emit only after AS. */
	bool sb_spin_lifecycle; /* Experimental: busy-wait lifecycle gate, writer priority. */
	bool sb_reader_preferred_lock; /* Ablation: original installer-preferring rwlock. */
	unsigned sb_prefetch_window; /* Claimed but not install-ACKed pages; zero is unbounded. */
	bool sb_fixed_ready_scan; /* Ablation: restart ready-slot selection from slot zero. */
	bool sb_serial_prefetch_install;
	bool sb_serial_background_install;
	bool sb_no_bg_fault_assist;
	bool sb_bg_round_robin; /* Ablation: rotate normal BG batch selection. */
	unsigned sb_fault_read_batch; /* Max original addresses extracted per lifecycle gate, 1..64. */
	unsigned sb_fault_install_workers; /* Per-PID demand installers; zero keeps the inline ablation. */
	bool sb_install_trace; /* Optional per-call target installation attribution. */
	char *sb_page_trace;
	unsigned sb_install_workers;
	unsigned sb_copy_workers;
