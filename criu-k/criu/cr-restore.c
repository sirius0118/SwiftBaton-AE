...
// 如果为真完成当前阶段，如果为假则切换到下一阶段无需等待其他任务完成
...
static int root_prepare_shared(void)
{
	...
	sb_trace("restore.shared_resources_begin");
	...
	for_each_pstree_item(pi) {
		if (pi->pid->state == TASK_HELPER)
			continue;
		// 这里有bug，应该是部分数据结构没有准备好。因为DOCKER宏的后遗症
	...
	show_saved_files();
err:
	sb_trace("restore.shared_resources_done");
	return ret;
}
...
static int restore_one_alive_task(int pid, CoreEntry *core)
{
	...
	sb_trace("restore.task_fds_begin");
	if (prepare_fds(current))
		return -1;
	sb_trace("restore.task_fds_done");

	if (prepare_file_locks(pid))
		return -1;

	sb_trace("restore.task_vmas_begin");
	if (open_vmas(current))
		return -1;
	sb_trace("restore.task_vmas_done");
...
static bool stage_needed_by_subtree(pid_t pid, void *opaque)
{
	struct pstree_item *item, *ancestor, *subtree = opaque;
...
static inline int fork_with_pid(struct pstree_item *item)
{
	...
	/* Stage pages belong in the restored tree, not in mount/iptables helpers. */
	sb_trace_task("restore.clone_prepare_begin", pid);
	if (opts.sb_parent_stage && sb_stage_prepare_fork(stage_needed_by_subtree, item)) {
		ret = -1;
		goto err_unlock;
	}
	sb_trace_task("restore.clone_begin", pid);
	if (kdat.has_clone3_set_tid) {
	...
	sb_trace_task("restore.clone_done", pid);
	if (opts.sb_parent_stage) {
		int saved_errno = errno;
		if (sb_stage_inherit(false)) {
			if (ret > 0 && item == root_item)
				item->pid->real = ret;
			ret = -1;
			goto err_unlock;
		}
	...
	sb_trace_task("restore.clone_cleanup_done", pid);
...
static int __restore_task_with_children(void *_arg)
{
	...
	current = ca->item;
	if (opts.sb_parent_stage && sb_stage_enter_child())
		goto err;
	if (sb_fork_restore_inheritance())
		goto err;
...
#ifdef DOCKER
	// DOCKERTODO: 在这里的代码之前，需要等待dump那边处理完namespace镜像
	// 然后这里再读取ns相关的镜像文件。
	...
	if (current->parent == NULL) {
		...
		// 等待当前阶段执行完，再进入下一阶段。因此就是先run_scripts之后，才会join对应的ns
		// 前面的prepare namespace相关的就都是用来read image的
		/* Wait prepare_userns */
	...
	// 这里是子进程会通过 set_ns 进入root_item创建的 mnt namespace
	if (restore_task_mnt_ns(current))
		goto err;

#ifdef DOCKER
	// DOCKERTODO: 这里开始需要对进程私有内容进行恢复，这里可以通过sync告诉dump要进行停机每个子进程dump了
	// 也要告诉restorer，要开始进行私有内容恢复了。restorer也就可以进行同步的相关信息的恢复了
#endif
	...
	if (prepare_private_fork(current))
		goto err;
	ret = create_children_and_session();
	if (sb_fork_restore_inheritance())
		goto err;
	if (ret)
		goto err;
...
#ifdef DOCKER
	/*DOCKERTODO: 这里放 prepare_pstree_noroot
	step1:等待root的通知，知道dump端已经进行停机，并且准备好了pstree.img
	接受root进程的信息，知道dump侧的进程已经停机，可以进行
	*/ 
#endif
...
int placehold(void)
{
    int ready[2], status, n;
    pid_t child;
    if (socketpair(AF_UNIX, SOCK_STREAM | SOCK_CLOEXEC, 0, ready))
        return -1;
    child = fork();
    if (child < 0) {
    ...
    if (!child) {
        close(ready[0]);
        status = unshare(CLONE_NEWNET) ? errno : 0;
        if (!status) sb_fdpool_populate(kdat.lsm != LSMTYPE__SELINUX);
        n = sync_transfer(ready[1], &status, sizeof(status), 1);
        close(ready[1]);
        if (status || n)
            _exit(EXIT_FAILURE);
        for (;;)
    ...
    n = sync_transfer(ready[0], &status, sizeof(status), 0);
    close(ready[0]);
    if (n || status) {
        kill(child, SIGKILL);
        waitpid(child, NULL, 0);
        pr_err("Network namespace placeholder failed: %d\n", n ? EIO : status);
        return -1;
    }
    /* A PID is not an FD: keep it out of service-fd relocation and close. */
    sb_netns_placeholder = child;
    return child;
}
...
int cr_restore_tasks(void)
{
	...
	if (ret < 0)
		goto err;
	sb_trace("restore.ps_mounts_ready");
	if (opts.sb_fd_placeholder && sb_fdpool_prepare(get_service_fd(IMG_FD_OFF))) goto err;
	nspid = placehold();
	if (nspid < 0)
		goto err;
	...
	sb_trace("restore.is_images_ready");
	if (opts.sb_parent_stage) {
		sb_trace("restore.stage_validate_begin");
		if (sb_stage_finalize(get_service_fd(IMG_FD_OFF))) goto err;
		sb_trace("restore.stage_validate_done");
	}
	// 告诉page-client，你可以开始读取pstree了
	update_state(page_sync, END_PROCESS_DUMP);

	// 在这里运行一个page-client，等待page-client启动完毕再进行后续操作
	// ret = run_page_client();
...
pr_warn("执行到这\n");
	...
	sb_trace("restore.root_task_end");
clean_cgroup:
	fini_cgroup();
err:
	/* Only signal the child created by this invocation. A missing service FD
	 * used to become kill(-1), and kill's return value hid restore failures. */
	if (nspid > 0 && waitpid(nspid, NULL, WNOHANG) == 0) {
		kill(nspid, SIGTERM);
		while (waitpid(nspid, NULL, 0) < 0 && errno == EINTR)
			;
	}
	fp = fopen("ready", "w");
	fputs("stop", fp);
...
static int sigreturn_restore(pid_t pid, struct task_restore_args *task_args, unsigned long alen, CoreEntry *core)
{
	...
	if (task_args->sbk_regions_nr)
		RST_MEM_FIXUP_PPTR(task_args->sbk_regions);
