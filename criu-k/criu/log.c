...
static void print_ts(char *line)
{
	struct timeval t;

	gettimeofday(&t, NULL);
	timediff(&start, &t);
	snprintf(line, TS_BUF_OFF, "(%02u.%06u", (unsigned)t.tv_sec, (unsigned)t.tv_usec);
	line[TS_BUF_OFF - 2] = ')'; /* this will overwrite the last digit if tv_sec>=100 */
	line[TS_BUF_OFF - 1] = ' '; /* kill the '\0' produced by snprintf */
}
...
static void vprint_on_level(unsigned int loglevel, const char *format, va_list params)
{
    ...
    /* The prefix is initialized before workers start; format each message
     * separately so concurrent dump threads cannot overwrite one another. */
    memcpy(line, buffer, buf_off);
	...
	size = vsnprintf(line + buf_off, sizeof line - buf_off, format, params);
    if (size < 0) { errno = _errno; return; }
    if (size >= sizeof line - buf_off) size = sizeof line - buf_off - 1;
	size += buf_off;

	while (off < size) {
		ret = write(fd, line + off, size - off);
		if (ret <= 0)
			break;
		off += ret;
	}
