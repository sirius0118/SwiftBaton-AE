...
void init_opts(void)
{
	...
	opts.sb_kernel_ps_chunk_mb = 64;
	opts.sb_kernel_export_workers = 1; /* Opt-in until physical comparison. */
...
#define BOOL_OPT(OPT_NAME, SAVE_TO)                         \
	...
	static struct option long_opts[] = {
		...
		{ "kernel-export-chunk-mb", required_argument, 0, 1255 },
		{ "kernel-device", required_argument, 0, 1249 },
		{ "kernel-gid", required_argument, 0, 1250 },
		{ "kernel-timeout-ms", required_argument, 0, 1251 },
		{ "image-rdma", no_argument, 0, 1210 },
		{ "u-precopy", no_argument, 0, 1211 },
		...
		{ "page-trace", required_argument, 0, 1227 },
		{ "serial-precopy-ack", no_argument, 0, 1228 },
		{ "sync-fault-transport", no_argument, 0, 1229 },
		{ "rdma-mtu", required_argument, 0, 1230 },
		...
		{ "serial-prefetch-install", no_argument, 0, 1235 },
		{ "serial-background-install", no_argument, 0, 1236 },
		{ "no-bg-fault-assist", no_argument, 0, 1237 },
		{ "bg-round-robin", no_argument, 0, 1238 },
		{ "install-trace", no_argument, 0, 1239 },
		{ "fault-install-workers", required_argument, 0, 1240 },
		{ "fault-read-batch", required_argument, 0, 1241 },
		{ "install-workers", required_argument, 0, 1216 },
		{ "batch-pages", required_argument, 0, 1217 },
		{ "copy-workers", required_argument, 0, 1218 },
		{ "fault-workers", required_argument, 0, 1222 },
...
#undef BOOL_OPT
	...
	while (1) {
		...
		case 1099:
			...
			break;
		case 1210:
			opts.sb_image_rdma = true;
			break;
		case 1215:
			opts.sb_parallel_transfer = true;
			break;
		case 1216:
			opts.sb_install_workers = atoi(optarg);
			if (!opts.sb_install_workers || opts.sb_install_workers > 32) return 1;
			break;
		case 1222:
            opts.sb_fault_workers = atoi(optarg);
            if (!opts.sb_fault_workers || opts.sb_fault_workers > 32) return 1;
            break;
        case 1223:
            opts.sb_prefetch_workers = atoi(optarg);
            if (!opts.sb_prefetch_workers || opts.sb_prefetch_workers > 32) return 1;
            break;
		case 1218:
			opts.sb_copy_workers = atoi(optarg);
			if (!opts.sb_copy_workers || opts.sb_copy_workers > 32) return 1;
			break;
		case 1217:
			opts.sb_batch_pages = atoi(optarg);
			if (!opts.sb_batch_pages || opts.sb_batch_pages > 256) return 1;
			break;
		case 1224: opts.sb_no_prefetch = true; break;
		case 1225: opts.sb_no_hot_first = true; break;
		case 1226: opts.sb_no_pretransfer = true; break;
		...
		case 1252: opts.sb_kernel_dense = true; break;
		case 1253:
			{ char *end; unsigned long value = strtoul(optarg, &end, 10);
			  if (!*optarg || *end || value > 4096) goto bad_arg;
			  opts.sb_kernel_ps_chunk_mb = value; }
			break;
        case 1254:
            { char *end; unsigned long value = strtoul(optarg, &end, 10);
              if (!*optarg || *end || value < 1 || value > 32) goto bad_arg;
              opts.sb_kernel_export_workers = value; }
            break;
        case 1255:
            { char *end; unsigned long value = strtoul(optarg, &end, 10);
              if (!*optarg || *end || value > 4096) goto bad_arg;
        ...
        case 1250:
            { char *end; unsigned long value = strtoul(optarg, &end, 10);
              if (!*optarg || *end || value > 255) goto bad_arg;
              opts.sb_kernel_gid = value; }
            break;
        case 1251:
            { char *end; unsigned long value = strtoul(optarg, &end, 10);
              if (!*optarg || *end || value < 10 || value > 30000) goto bad_arg;
              opts.sb_kernel_timeout_ms = value; }
            break;
		...
		case 1243:
			...
			break;
		case 1232: opts.sb_reader_preferred_lock = true; break;
		case 1234: opts.sb_fixed_ready_scan = true; break;
		case 1235: opts.sb_serial_prefetch_install = true; break;
		case 1236: opts.sb_serial_background_install = true; break;
		case 1237: opts.sb_no_bg_fault_assist = true; break;
		case 1238: opts.sb_bg_round_robin = true; break;
		case 1239: opts.sb_install_trace = true; break;
		case 1241:
			opts.sb_fault_read_batch = atoi(optarg);
			if (!opts.sb_fault_read_batch || opts.sb_fault_read_batch > 64) goto bad_arg;
			break;
		case 1240:
			opts.sb_fault_install_workers = atoi(optarg);
			if (opts.sb_fault_install_workers > 32) goto bad_arg;
			break;
		case 1233:
			opts.sb_prefetch_window = atoi(optarg);
			if (opts.sb_prefetch_window > 4096) goto bad_arg;
			break;
		case 1230:
			opts.sb_rdma_mtu = atoi(optarg);
			if (opts.sb_rdma_mtu != 256 && opts.sb_rdma_mtu != 512 &&
			    opts.sb_rdma_mtu != 1024 && opts.sb_rdma_mtu != 2048 && opts.sb_rdma_mtu != 4096)
				return 1;
			break;
		case 1214:
		...
		case 1220:
			...
			if (!opts.sb_validation_workers || opts.sb_validation_workers > 32) return 1;
			break;
		case 1212:
			opts.sb_precopy_workers = atoi(optarg);
			if (!opts.sb_precopy_workers || opts.sb_precopy_workers > 32) return 1;
			break;
		case 1213:
			opts.sb_precopy_limit_mb = atoi(optarg);
			if (!opts.sb_precopy_limit_mb || opts.sb_precopy_limit_mb > 65536) return 1;
			break;
		case 1100:
