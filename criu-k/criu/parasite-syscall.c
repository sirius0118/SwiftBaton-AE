...
static void sigchld_handler(int signal, siginfo_t *siginfo, void *data)
{
    ...
    if (enter_multi_process) {
        ...
        bool tracee = false;
        /* waitpid(-1) here steals ptrace stops from another dump worker.
         * The owning worker must consume its own stop with an explicit PID.
         * Other CRIU subprocesses are likewise reaped by their own caller. */
        for_each_pstree_item(item) {
            for (int t = 0; t < item->nr_threads; t++)
                if (item->threads[t].real == siginfo->si_pid) tracee = true;
        }
        if (!tracee || siginfo->si_code == CLD_CONTINUED) return;
        if ((siginfo->si_code == CLD_TRAPPED || siginfo->si_code == CLD_STOPPED) &&
            ((siginfo->si_status & 0x7f) == SIGTRAP || siginfo->si_status == SIGSTOP)) return;
        pr_err("Parallel tracee %d failed: code=%d status=%d\n",
			      ...
			      struct fd_opts *opts)
{
	...
	pr_info("SB_FD drain_request count=%d\n", nr_fds);
	...
	pr_info("SB_FD drain_received count=%d rc=%d\n", nr_fds, ret);
	if (ret)
...
struct parasite_ctl *parasite_infect_seized(pid_t pid, struct pstree_item *item, struct vm_area_list *vma_area_list)
{
	...
	parasite_ensure_args_size(sizeof(struct sbk_export_args));
