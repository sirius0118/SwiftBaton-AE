...
/* 在pie里面进行vma的重新映射,除了premap的vma和参数vma,其余所有的区域都unregistered*/
...
static int timerfd_wait_expired(int fd)
{
	struct pollfd pfd = { fd, POLLIN, 0 };
	struct timespec timeout = { 1, 0 };
	int ret = sys_ppoll(&pfd, 1, &timeout, NULL, sizeof(sigset_t));

	return ret == 1 && (pfd.revents & POLLIN) ? 0 : ret < 0 ? ret : -ETIMEDOUT;
}

#define SB_TFD_CLOCK sys_clock_gettime
#define SB_TFD_SET(fd, flags, val) sys_timerfd_settime(fd, flags, val, NULL)
#define SB_TFD_WAIT timerfd_wait_expired
#define SB_TFD_TICKS(fd, ticks) sys_ioctl(fd, TFD_IOC_SET_TICKS, (unsigned long)(ticks))
#include "sb-timerfd-restore.inc"
#undef SB_TFD_CLOCK
#undef SB_TFD_SET
#undef SB_TFD_WAIT
#undef SB_TFD_TICKS
...
static int timerfd_arm(struct task_restore_args *args)
{
	...
	for (i = 0; i < args->timerfd_n; i++) {
		...
		struct sb_timerfd_state state = {
			.clockid = t->clockid, .flags = t->settime_flags,
			.ticks = t->ticks, .val = t->val,
			.has_realtime = t->has_realtime, .realtime = t->realtime,
		};
		int ret = sb_restore_timerfd(t->fd, &state);

		if (ret) {
			pr_err("Can't restore timerfd %d (id %x): %d\n", t->fd, t->id, ret);
			return ret;
		}
	}
	return 0;
...
__visible long __export_restore_task(struct task_restore_args *args)
{
	...
	*args->breakpoint = rst_sigreturn;
	...
	for (i = args->vmas_n - 1; i >= 0; i--) {
		...
		if (vma_entry_is(vma_entry, VMA_AREA_SHSTK))
			continue;
		// 如果页面要延迟加载,那么就会有相应的uffd
		if (vma_remap(vma_entry, args->uffd))
			goto core_restore_end;
	}
	/* K arms only after all temporary private VMAs reached final addresses.
	 * A failed ARM must abort restore, never resume a partly armed process. */
	if (args->sbk_regions_nr) {
		if (args->uffd >= 0
#ifdef MUL_UFFD
		    || args->uffd_set.nr_uffd_region
#endif
		   ) {
			pr_err("SwiftBaton-K cannot coexist with UFFD in this restorer\n");
			goto core_restore_end;
		}
        /* A child may inherit a parent's restored bytes in a range that was
        ...
        for (i = 0; i < args->vmas_n; i++) {
            vma_entry = args->vmas + i;
            if (vma_entry_can_be_lazy(vma_entry) &&
                sys_madvise(vma_entry->start, vma_entry->end - vma_entry->start, MADV_DONTNEED)) {
                pr_err("SwiftBaton-K cannot reset lazy anonymous range\n");
                goto core_restore_end;
            }
        }
...
#ifdef MUL_UFFD
	if (!args->sbk_regions_nr && enable_uffdset(args) < 0)
		goto core_restore_end;
	/* The page client owns the SCM_RIGHTS copies. Drop every restorer fd
	 * so the pager can end registration by closing its final references. */
	for (i = 0; i < args->uffd_set.nr_uffd_region; i++)
		sys_close(args->uffd_set.uffd_region[i].uffd);
	if (args->sb_uffd_ready_fd >= 0) {
		uint64_t ready = 1;
		if (sys_write(args->sb_uffd_ready_fd, &ready, sizeof(ready)) != sizeof(ready)) {
			pr_err("Cannot notify page client that lazy VMAs are registered\n");
			goto core_restore_end;
		}
		sys_close(args->sb_uffd_ready_fd);
		args->sb_uffd_ready_fd = -1;
	}
#endif
	// load_page
	...
	pr_debug("SB_RESTORE_STAGE memory_layout_done\n");
	/*
	...
	pr_debug("SB_RESTORE_STAGE protection_and_aio_done\n");
	/*
	 ...
	 */
	pr_debug("SB_RESTORE_STAGE madvise_done exe_fd=%d\n",args->fd_exe_link);
	...
	pr_debug("SB_RESTORE_STAGE mm_map_begin\n");
	ret = sys_prctl(PR_SET_MM, PR_SET_MM_MAP, (long)&prctl_map, sizeof(prctl_map), 0);
	pr_debug("SB_RESTORE_STAGE mm_map_done rc=%ld\n",ret);
	if (ret) {
	...
	if (ret == -EINVAL) {
		...
		pr_debug("SB_RESTORE_STAGE exe_close_begin fd=%d\n",args->fd_exe_link);
		sys_close(args->fd_exe_link);
		pr_debug("SB_RESTORE_STAGE exe_close_done\n");
	}
...
#define itimer_armed(args, i) (args->itimers[i].it_interval.tv_sec || args->itimers[i].it_interval.tv_usec)
	...
	// 这里将 premap 的内存,所有vma的数据进行 unmap
	 ...
	 * code insns from gcc.
	 */
	// 这里的rt_sigframe里面包含恢复目标进程的寄存器等信息
	rst_sigreturn(new_sp, rt_sigframe);
