...
static void trace_drain_fds(const char *phase)
{
	struct timespec mono = {0}, real = {0};
	if (sys_clock_gettime(CLOCK_MONOTONIC_RAW, &mono) || sys_clock_gettime(CLOCK_REALTIME, &real))
		return;
	pr_info("SB_PHASE phase=%s pid=%ld mono_ns=%llu real_ns=%llu\n", phase, sys_getpid(),
...
static int sbk_export_regions(struct sbk_export_args *args)
{
	int fd = recv_fd(parasite_get_rpc_sock());
	int ret;
	if (fd < 0)
		return fd;
	ret = sbk_pie_export(fd, args);
	sys_close(fd);
	return ret;
}
...
int parasite_daemon_cmd(int cmd, void *args)
{
	...
	switch (cmd) {
	case PARASITE_CMD_SBK_EXPORT:
		ret = sbk_export_regions(args);
		break;
