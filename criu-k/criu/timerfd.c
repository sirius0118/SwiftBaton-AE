...
static int read_timerfd_state(int fd, struct sb_timerfd_state *s)
{
	TimerfdEntry tfe = TIMERFD_ENTRY__INIT;
	int ret = parse_fdinfo(fd, FD_TYPES__TIMERFD, &tfe);
	if (ret)
		return ret;
	/* Reject impossible image values before narrowing to time_t/long. */
	if (tfe.vsec > INT64_MAX / SB_TFD_NSEC || tfe.isec > INT64_MAX / SB_TFD_NSEC ||
	    tfe.vnsec >= SB_TFD_NSEC || tfe.insec >= SB_TFD_NSEC)
		return -EINVAL;
	s->clockid = tfe.clockid;
	s->flags = tfe.settime_flags;
	s->ticks = tfe.ticks;
	s->val.it_value = (struct timespec){ tfe.vsec, tfe.vnsec };
	s->val.it_interval = (struct timespec){ tfe.isec, tfe.insec };
	return 0;
}

#define SB_TFD_READ read_timerfd_state
#define SB_TFD_GET timerfd_gettime
#define SB_TFD_CLOCK clock_gettime
#include "sb-timerfd-capture.inc"
#undef SB_TFD_READ
#undef SB_TFD_GET
#undef SB_TFD_CLOCK
...
static int dump_one_timerfd(int lfd, u32 id, const struct fd_parms *p)
{
	...
	struct sb_timerfd_state state = { 0 };
	int ret;

	ret = sb_capture_timerfd(lfd, &state);
	if (ret) {
		pr_err("Cannot capture timerfd %#x consistently: %d (cancel-on-set state is unsupported)\n", id, ret);
		return -1;
	}
	tfe.clockid = state.clockid;
	tfe.settime_flags = state.flags;
	tfe.ticks = state.ticks;
	tfe.vsec = state.val.it_value.tv_sec;
	tfe.vnsec = state.val.it_value.tv_nsec;
	tfe.isec = state.val.it_interval.tv_sec;
	tfe.insec = state.val.it_interval.tv_nsec;
	tfe.has_sb_realtime_sec = tfe.has_sb_realtime_nsec = state.has_realtime;
	tfe.sb_realtime_sec = state.realtime.tv_sec;
	tfe.sb_realtime_nsec = state.realtime.tv_nsec;
...
int prepare_timerfds(struct task_restore_args *ta)
{
	...
	list_for_each_entry(ti, &rst_timerfds, rlist) {
		...
		t->has_realtime = tfe->has_sb_realtime_sec;
		t->realtime = (struct timespec){ tfe->sb_realtime_sec, tfe->sb_realtime_nsec };
...
static int collect_one_timerfd(void *o, ProtobufCMessage *msg, struct cr_img *i)
{
	...
	if (info->tfe->has_sb_realtime_sec != info->tfe->has_sb_realtime_nsec ||
	    info->tfe->vsec > INT64_MAX / SB_TFD_NSEC || info->tfe->isec > INT64_MAX / SB_TFD_NSEC ||
	    info->tfe->vnsec >= SB_TFD_NSEC || info->tfe->insec >= SB_TFD_NSEC ||
	    (info->tfe->has_sb_realtime_sec &&
	     (info->tfe->clockid != CLOCK_REALTIME || !(info->tfe->settime_flags & TFD_TIMER_ABSTIME) ||
	      (info->tfe->settime_flags & SB_TFD_CANCEL_ON_SET) ||
	      info->tfe->sb_realtime_sec > INT64_MAX / SB_TFD_NSEC || info->tfe->sb_realtime_nsec >= SB_TFD_NSEC))) {
		pr_err("Invalid timerfd times/snapshot for %#x\n", info->tfe->id);
		return -1;
	}
