...
static pthread_mutex_t pipe_dump_lock = PTHREAD_MUTEX_INITIALIZER;

static int dump_one_pipe_data_locked(struct pipe_data_dump *pd, int lfd, const struct fd_parms *p)
{
	struct cr_img *img;
	...
	while (bytes > 0) {
		...
		if (wrote < 0) {
			...
			pr_err("Unexpected EOF copying pipe data\n");
			goto err_close;
		}
		bytes -= wrote;
	}
...
{
	int ret;
	/* Deduplication and header+raw-splice form one transaction, including
	 * when several processes share the same pipe or FIFO. */
	pthread_mutex_lock(&pipe_dump_lock);
	ret = dump_one_pipe_data_locked(pd, lfd, p);
	pthread_mutex_unlock(&pipe_dump_lock);
	return ret;
}
