...
#include <pthread.h>
static pthread_mutex_t sk_queue_image_lock = PTHREAD_MUTEX_INITIALIZER;

static int write_sk_packet(SkPacketEntry *pe, void *data)
{
	int ret;
	/* SCM discovery above this call can recursively dump other descriptors.
	 * Only serialize the image append, after all recursive work has ended. */
	pthread_mutex_lock(&sk_queue_image_lock);
	...
	pthread_mutex_unlock(&sk_queue_image_lock);
	return ret;
}
