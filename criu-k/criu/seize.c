...
int pstree_finish_freezer(int st)
{
	int ret = st == TASK_DEAD ? 0 : freezer_restore_state();
	int detach_ret = freezer_detach();
	return ret ? ret : detach_ret;
}
...
void pstree_switch_state(struct pstree_item *root_item, int st)
{
	 ...
	 * 
	 * 有些进程是也被一起暂停了，因为在同一个cgroup下面，但是这些进程不需要进行dump，这些进程需要被恢复
	 */
	freezer_detach();
