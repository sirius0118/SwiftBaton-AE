...
/* A worker must not lend the same buffer to a procfs reader and an image
 * writer running in another thread. Buffers returned by another thread can
 * safely join this thread's free list after their previous owner closes. */
static __thread struct list_head bufs;
static __thread bool bufs_initialized;
static pthread_mutex_t image_write_lock = PTHREAD_MUTEX_INITIALIZER;
static void init_buf_pool(void)
{
    if (!bufs_initialized) {
        INIT_LIST_HEAD(&bufs);
...
{
	return __atomic_load_n(&flush_failed, __ATOMIC_ACQUIRE) ? -1 : 0;
}
...
void bclose(struct bfd *f)
{
	if (bfd_buffered(f)) {
		if (f->writable && bflush(f) < 0) {
			 ...
			 */
			__atomic_store_n(&flush_failed, true, __ATOMIC_RELEASE);
...
static int bwrite_unlocked(struct bfd *bfd, const void *buf, int size)
{
	if (!bfd_buffered(bfd))
		return write_all(bfd->fd, buf, size);
...
static int bwritev_unlocked(struct bfd *bfd, const struct iovec *iov, int cnt)
{
	int i, written = 0;
...
/* A protobuf record consists of length and payload iovecs. Some images are
 * shared by all dump tasks, so preserve that record as one buffered append. */
int bwrite(struct bfd *bfd, const void *buf, int size)
{
    int ret;
    pthread_mutex_lock(&image_write_lock);
    ret = bwrite_unlocked(bfd, buf, size);
    pthread_mutex_unlock(&image_write_lock);
    return ret;
}
int bwritev(struct bfd *bfd, const struct iovec *iov, int cnt)
{
    int ret;
    pthread_mutex_lock(&image_write_lock);
    ret = bwritev_unlocked(bfd, iov, cnt);
    pthread_mutex_unlock(&image_write_lock);
    return ret;
}
