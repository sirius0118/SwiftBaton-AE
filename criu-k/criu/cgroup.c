...
int dump_thread_cgroup(const struct pstree_item *item, u32 *cg_id, struct parasite_dump_cgroup_args *args, int id)
{
	...
	// 这里是判断CRIU是否允许在特权模式中，非特权模式能做的功能应该比较少
	if (opts.unprivileged)
		return 0;
...
#ifdef PARALLEL_DUMP
	// 获得item对应的cs。这个操作需要互斥执行，因为涉及到了cs_sets数组的创建
	// 保证root_item线程先进入临界区
	// 借用两种lock来实现root线程先进入，然后所有子线程互斥访问。前面也会调用到这个函数，不仅仅是多线程的时候才会使用。
