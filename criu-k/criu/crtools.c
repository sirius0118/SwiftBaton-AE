...
usage:
	...
	pr_msg("\n"
	       ...
	       "  --kernel-transfer     experimental in-kernel RDMA page transport\n"
	       "  --kernel-device DEV   kernel RDMA device (default mlx5_1)\n"
	       "  --kernel-dense       ablation: retain whole lazy VMA registration\n"
	       "  --kernel-gid INDEX    RDMA GID index (default 0)\n"
	       "  --kernel-timeout-ms N kernel RDMA timeout, 10..30000 (default 2000)\n"
	       "  --kernel-export-workers N final MR registration workers, 1..32 (default 1)\n"
	       "  --kernel-export-chunk-mb N final MR span target, 1..4096 MiB (0 legacy)\n"
	       "  --kernel-ps-chunk-mb N independently publish PS spans, 1..4096 MiB (default 64; 0 legacy)\n"
		   ...
		   "  -m|--mem-predict		method of memory prediction(None, Used, LRU)\n"
