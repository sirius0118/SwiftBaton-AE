...
{
	int pfd[2], ret = -1;
	/* The legacy frontend otherwise retries the global xtables lock only
	 * once per second. Keep the lock, but avoid a whole second of network
	 * blackout when IPv4 commits before IPv6 acquires that lock. */
	char *cmd4[] = { "iptables-restore", "-w", "10", "-W", "1000", "--noflush", NULL };
	char *cmd6[] = { "ip6tables-restore", "-w", "10", "-W", "1000", "--noflush", NULL };
	...
	sb_trace(ipv6 ? "network.ip6tables_begin" : "network.iptables_begin");
	ret = cr_system(pfd[0], -1, -1, cmd[0], cmd, 0);
	sb_trace(ipv6 ? "network.ip6tables_done" : "network.iptables_done");
err:
	close_safe(&pfd[1]);
	close_safe(&pfd[0]);
	return ret;
...
int network_lock_internal(void)
{
	...
	sb_trace("network.enter_namespace_begin");
	if (switch_ns(root_item->pid->real, &net_ns_desc, &nsret))
		return -1;

	sb_trace("network.enter_namespace_done");
	...
	sb_trace("network.rules_installed");
	if (restore_ns(nsret, &net_ns_desc))
		ret = -1;

	sb_trace("network.exit_namespace_done");
	return ret;
}
...
int network_lock(void)
{
	...
	sb_trace("network.rpc_begin");
	if (run_scripts(ACT_NET_LOCK))
		return -1;
	sb_trace("network.rpc_done");
