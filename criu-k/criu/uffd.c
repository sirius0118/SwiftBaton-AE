...
int setup_uffdset(int pid, struct task_restore_args *task_args)
{
    ...
    if (opts.sb_kernel_transfer) {
        int ret;
        if (page_server_sk < 0) return -1;
        sb_uffd_send_lock();
        ret = sb_kernel_receive_restore(task_args, page_server_sk, pid);
        sb_uffd_send_unlock();
...
int cr_lazy_pages(bool daemon)
{
	...
	if (!opts.sb_kernel_transfer && !kdat.has_uffd)
		return -1;
	pr_warn("cr_lazy_pages: create page client\n ");
#ifdef DOCKER
    if (opts.sb_kernel_transfer) {
        if (sb_kernel_options() || daemon) return -1;
        ...
        close(page_sync);close(sync_fd_PC);
        return ret;
    }
