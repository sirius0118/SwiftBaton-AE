        ...
        if (!r || r->source_pid != (pid_t)page->pid || r->start != page->region_start || r->end != page->region_end) {
            ...
            *r = (struct stage_region){ .source_pid = page->pid, .start = page->region_start,
            ...
            if (r->mapping == MAP_FAILED) {
                ...
                r->mapping = NULL;
            } else if (sb_numa_bind(r->mapping, r->end - r->start, stage_numa_node)) {
                /* An explicitly requested policy must not silently fail. */
                pr_perror("Bind anonymous PS stage to requested NUMA node");
                goto fail;
            }
        }
