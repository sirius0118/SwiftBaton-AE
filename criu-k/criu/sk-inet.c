...
static int open_inet_sk(struct file_desc *d, int *new_fd)
{
	...
	/* Only empty, context-matched PS objects; final addresses, queues,
	 * TCP repair and options below remain authoritative. */
	int pool_kind = sb_fdpool_socket_kind(ie->family, ie->type, ie->proto);
	sk = pool_kind >= 0 && !fle->fe->xattr_security_selinux ? sb_fdpool_take(pool_kind) : -1;
	if (sk < 0) sk = socket(ie->family, ie->type, ie->proto);
	if (sk < 0) {
