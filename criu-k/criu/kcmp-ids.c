...
static uint32_t kid_generate_gen_locked(struct kid_tree *tree, struct kid_elem *elem, int *new_id)
{
...
static struct kid_elem *kid_lookup_epoll_tfd_locked(struct kid_tree *tree, struct kid_elem *elem, kcmp_epoll_slot_t *slot)
{
	struct rb_node *node = tree->root.rb_node;
	struct rb_node **new = &tree->root.rb_node;
...
/* Parallel dump workers share these registries. Protect only identity lookup
 * and publication; object inspection, parasite RPC and image I/O stay outside.
 * Each published elem is immutable and lives until the coordinator exits,
 * so an epoll lookup may safely return a pointer after releasing this lock. */
uint32_t kid_generate_gen(struct kid_tree *tree, struct kid_elem *elem, int *new_id)
{
    uint32_t id;
    pthread_mutex_lock(&tree->lock);
    id = kid_generate_gen_locked(tree, elem, new_id);
    pthread_mutex_unlock(&tree->lock);
    return id;
}
uint32_t kid_generate_id(struct kid_tree *tree)
{
    uint32_t id;
    pthread_mutex_lock(&tree->lock);
    id = tree->subid++;
...
struct kid_elem *kid_lookup_epoll_tfd(struct kid_tree *tree, struct kid_elem *elem,
                                    kcmp_epoll_slot_t *slot)
{
    struct kid_elem *found;
    pthread_mutex_lock(&tree->lock);
    found = kid_lookup_epoll_tfd_locked(tree, elem, slot);
    pthread_mutex_unlock(&tree->lock);
    return found;
}
