...
/* Deferred references belong to the current FD-dump actor. Another worker
 * must not publish/free them before this actor has indexed its remaining FDs. */
static __thread struct list_head dinfo_list;
static struct list_head *pending_dinfo(void)
{
    if (!dinfo_list.next)
        INIT_LIST_HEAD(&dinfo_list);
    return &dinfo_list;
}
...
static int queue_dinfo(FileEntry **fe, EventpollFileEntry **e, toff_t **toff, const struct fd_parms *p)
{
	...
	*toff = NULL;
	...
	list_add_tail(&dinfo->list, pending_dinfo());
	return 0;
}
...
int flush_eventpoll_dinfo_queue(void)
{
	...
	list_for_each_entry_safe(dinfo, t, pending_dinfo(), list) {
...
static int eventpoll_open(struct file_desc *d, int *new_fd)
{
	...
	tmp = sb_fdpool_take(SB_FDP_EPOLL);
	if (tmp < 0) tmp = epoll_create(1);
	if (tmp < 0) {
