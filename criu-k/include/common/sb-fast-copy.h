...
static inline int sb_fast_copy_one(volatile struct sb_fast_queue *q, unsigned *cursor, unsigned worker)
{
    for (unsigned i = 0; i < SB_FAST_SLOTS; i++) {
        ...
        volatile struct sb_fast_slot *s = &q->slots[slot];
        /* Empty/busy slots vastly outnumber requests on the fault lane. Poll
         * them read-only: even a failed locked CAS obtains write ownership of
         * the cache line and contends with the coordinator and other workers.
         * The CAS remains the sole ownership claim and acquire operation. */
        if (__atomic_load_n(&s->state, __ATOMIC_RELAXED) != SB_FAST_REQUEST) continue;
