...
*/
  ...
  public ScrambledZipfianGenerator(long min, long max, double zipfianconstant) {
    ...
    // } 
    double ZETAN = 0, USED_ZIPFIAN_CONSTANT = 0;
    double zeta[] = {0.5, 0.7, 0.9, 0.99, 1.11, 1.22};
    double zetan[] = {199998.539650495862588, 3330.554944937512118, 90.569885981112165, 26.469028201746227, 8.953958853571397, 5.109771888935082};
    for (int i = 0; i < 6; i++) {
      if (zipfianconstant == zeta[i]) {
        ZETAN = zetan[i];
        USED_ZIPFIAN_CONSTANT = zeta[i];
        break;
      }
    }
    System.err.println("zipfianconstant: " + zipfianconstant + " ZETAN: " + ZETAN + " USED_ZIPFIAN_CONSTANT: " + USED_ZIPFIAN_CONSTANT);
    gen = new ZipfianGenerator(0, ITEM_COUNT, USED_ZIPFIAN_CONSTANT, ZETAN);
    // this.itemcount = 0;
  }
